...
# Paths permitted to import from job_search_mcp_server.* directly.
ALLOWED_PREFIXES = (
    SRC_ROOT / "job_search_mcp_server",  # the vendored package itself
    SRC_ROOT / "scraping" / "adapters",  # adapter-local shims
)

# Compatibility shim that re-exports from mcp_interface (and may transitively
# touch job_search_mcp_server only through that import). The shim itself does not
# import job_search_mcp_server directly.
IMPORT_RE = re.compile(r"^\s*(?:from\s+job_search_mcp_server|import\s+job_search_mcp_server)", re.MULTILINE)
...
def test_no_vendored_imports_outside_adapter_boundary() -> None:
    ...
    assert not offenders, (
        "The following modules import from job_search_mcp_server.* outside the "
...
def test_orchestrator_wrappers_do_not_leak_vendored_imports(module_path: str) -> None:
    ...
    assert not IMPORT_RE.search(text), (
        ...
        "from job_search_mcp_server directly."
    )
