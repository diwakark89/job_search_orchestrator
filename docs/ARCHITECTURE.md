...
│   └── job_search_mcp_server/       # Vendored compatibility package from the former sister repo
└── tests/                       # Unified test suite
```

## Merge Strategy

The first implementation step keeps `src/job_search_mcp_server/` vendored to preserve MCP behavior and standalone CLI compatibility without rewriting all internal imports immediately.
...
Future refactors should migrate internals from `src/job_search_mcp_server/` into `src/scraping/` gradually, then reduce the vendored package to compatibility wrappers only.
