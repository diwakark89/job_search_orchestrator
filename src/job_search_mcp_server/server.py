#!/usr/bin/env python3
"""
Job Search MCP Server - 2025 Latest Implementation

An MCP server that provides job scraping capabilities using the Job Search library.
...
# Job Search imports
from job_search_mcp_server.json_output import (
...
from job_search_mcp_server.job_search_scrapers import scrape_jobs
from job_search_mcp_server.job_search_scrapers.model import Country
from job_search_mcp_server.preferences import derive_runtime_defaults, load_search_preferences
from job_search_mcp_server.markdown_output import (
...
from job_search_mcp_server.guardrails import (
    CITIES_DEFAULT,
    CITIES_MAX,
    CITIES_MIN,
...
async def scrape_jobs_tool(
    ...
    try:
        ...
        # Call Job Search scrape_jobs function
        jobs_df = scrape_jobs(
            site_name=site_name,
    ...
    try:
        sites_info = [
            ...
            },
            {
                ...
                "reliability_note": "Backed by a public Algolia search API; disabled when JOB_SEARCH_RESPECT_ROBOTS is set.",
            },
            {
                "name": "eu_startups",
            ...
            },
            {
                ...
                "regions": ["germany", "dach", "europe"],
                "reliability_note": "Disabled when JOB_SEARCH_RESPECT_ROBOTS is set (ToS-restricted).",
            },
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Job Search MCP Server — job scraping as AI-callable tools"
    )
    parser.add_argument(
        "--transport",
