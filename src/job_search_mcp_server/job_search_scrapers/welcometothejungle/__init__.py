...
from job_search_mcp_server.job_search_scrapers.exception import WelcomeToTheJungleException
...
from job_search_mcp_server.job_search_scrapers.welcometothejungle.constant import (
    ALGOLIA_API_KEY,
    ALGOLIA_APP_ID,
    ALGOLIA_INDEX,
    algolia_headers,
...
from job_search_mcp_server.job_search_scrapers.welcometothejungle.util import (
    map_country_to_code,
    parse_iso_date,
    truthy_env,
)
...
class WelcomeToTheJungleScraper(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        if truthy_env(os.getenv("JOB_SEARCH_RESPECT_ROBOTS")):
            log.warning(
                "JOB_SEARCH_RESPECT_ROBOTS is set; skipping Welcome to the Jungle (ToS-restricted)."
            )
