...
from job_search_mcp_server.job_search_scrapers.glassdoor.constant import fallback_token, query_template, headers
from job_search_mcp_server.job_search_scrapers.glassdoor.util import (
    get_cursor_for_page,
    parse_compensation,
    parse_location,
)
...
from job_search_mcp_server.job_search_scrapers.exception import GlassdoorException
...
class Glassdoor(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        if not job_list and self.last_error:
            raise GlassdoorException(self.last_error)
    ...
    def _fetch_jobs_page(
        ...
        try:
            ...
            self.last_error = f"Glassdoor API failure: {str(e)}"
            log.error(f"Glassdoor: {str(e)}")
            return jobs, None
