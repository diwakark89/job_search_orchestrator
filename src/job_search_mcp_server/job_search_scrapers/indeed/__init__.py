...
from job_search_mcp_server.job_search_scrapers.indeed.constant import job_search_query, api_headers
from job_search_mcp_server.job_search_scrapers.indeed.util import is_job_remote, get_compensation, get_job_type
from job_search_mcp_server.job_search_scrapers.exception import IndeedException
...
class Indeed(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        if not job_list and self.last_error:
            raise IndeedException(self.last_error)
    ...
    def _scrape_page(self, cursor: str | None) -> Tuple[list[JobPost], str | None]:
        ...
        if self.scraper_input.location:
            escaped_location = self.scraper_input.location.replace('"', '\\"')
            # Indeed GraphQL currently requires radius whenever location is provided.
            radius = self.scraper_input.distance if self.scraper_input.distance is not None else 50
            location_clause = (
                f'location: {{where: "{escaped_location}", radius: {radius}, radiusUnit: MILES}}'
            )
        ...
        if not response.ok:
            self.last_error = f"Indeed API failure: HTTP {response.status_code}"
            log.info(
            ...
            try:
                data = response.json()
                if data.get("errors"):
                    self.last_error = f"Indeed API failure: {data['errors'][0].get('message', 'unknown error')}"
            except Exception:
                pass
        ...
        if data.get("errors"):
            self.last_error = f"Indeed API failure: {data['errors'][0].get('message', 'unknown error')}"
            log.error("Indeed: API errors returned: %s", data["errors"])
            return jobs, new_cursor

        job_search = data.get("data", {}).get("jobSearch")
        if not job_search:
            self.last_error = "Indeed API failure: Missing jobSearch payload in API response"
            log.error("Indeed: Missing jobSearch payload in API response")
            return jobs, new_cursor
    ...
    def _process_job(self, job: dict) -> JobPost | None:
        ...
        return JobPost(
            ...
            description_source="listing_api" if description else None,
