...
from job_search_mcp_server.job_search_scrapers.exception import JoinException
from job_search_mcp_server.job_search_scrapers.join.constant import headers
from job_search_mcp_server.job_search_scrapers.join.util import (
    parse_location,
    parse_post_date,
    truthy_env,
)
...
class JoinScraper(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        if truthy_env(os.getenv("JOB_SEARCH_RESPECT_ROBOTS")):
            log.warning("JOB_SEARCH_RESPECT_ROBOTS is set; skipping join.com (ToS-restricted).")
