...
from job_search_mcp_server.job_search_scrapers.model import CompensationInterval, JobType, Site

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)


def create_logger(name: str):
    logger = logging.getLogger(f"Job Search:{name}")
...
def set_logger_level(verbose: int):
    ...
    if level is not None:
        for logger_name in logging.root.manager.loggerDict:
            if logger_name.startswith("Job Search:"):
...
def normalize_job_type_value(value: str | None) -> str | None:
    """Normalize free-form job type values to canonical API values."""
    if not value:
        return None
    ...
    for token in tokens:
        ...
        mapped = canonical_map.get(normalized)
        if mapped is not None:
            return mapped
    return None


def infer_work_mode(*, work_from_home_type: str | None, is_remote: bool | None, title: str | None, description: str | None, location: str | None) -> str | None:
    """Infer canonical work mode from available signals."""
    for raw in (work_from_home_type, title, description, location):
        candidate = normalize_work_mode(raw)
...
def normalize_work_mode(value: str | None) -> str | None:
    ...
    remote_pattern = re.compile(
        r"\b(remote|remotely|work from home|wfh)\b",
        re.IGNORECASE,
    )
    onsite_pattern = re.compile(
        r"\b(on-site|onsite|on site|office|in-office|work from office)\b",
        re.IGNORECASE,
    )

    has_remote = remote_pattern.search(text) is not None
    has_onsite = onsite_pattern.search(text) is not None
