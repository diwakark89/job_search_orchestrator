...
from job_search_mcp_server.job_search_scrapers import scrape_jobs as _scrape_jobs

from scraping.output import build_jobs_json_payload


class JobSearchMcpAdapter:
    """Concrete ScraperPort implementation backed by the vendored job_search_mcp_server package.

    This adapter is the single point of coupling between the orchestrator service layer
    and the underlying Job Search scraper library.  Changing the vendored package only
    ...
    def search(
        ...
        *,
        ...
        """Invoke the vendored job-search scraper and return normalised (jobs, site_errors).

        Raises:
            RuntimeError: Wraps any exception raised by the underlying scraper so
                callers do not need to handle job-search-specific exception types.
        """
        try:
