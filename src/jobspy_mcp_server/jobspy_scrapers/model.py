...
_DESCRIPTION_SOURCE_ALIASES: dict[str, DescriptionSource] = {
    "listing": DescriptionSource.LISTING_API,
    "listing_api": DescriptionSource.LISTING_API,
    "listingapi": DescriptionSource.LISTING_API,
    "listing_page": DescriptionSource.LISTING_API,
    "api": DescriptionSource.LISTING_API,
    "detail": DescriptionSource.DETAIL_PAGE,
    "detail_page": DescriptionSource.DETAIL_PAGE,
    "detailpage": DescriptionSource.DETAIL_PAGE,
    "details": DescriptionSource.DETAIL_PAGE,
    "job_page": DescriptionSource.DETAIL_PAGE,
}


def normalize_description_source(value: DescriptionSource | str | None) -> str | None:
    """Normalize free-form description source values to a strict canonical set."""
    if value is None:
        return None

    if isinstance(value, DescriptionSource):
        return value.value
    ...
    canonical = _DESCRIPTION_SOURCE_ALIASES.get(normalized)
    if canonical is not None:
        return canonical.value

    return normalized if normalized in DescriptionSource._value2member_map_ else None
...
class JobPost(BaseModel):
    ...
    @classmethod
    def _normalize_description_source(cls, value: DescriptionSource | str | None) -> str | None:
        return normalize_description_source(value)
...
class ScraperInput(BaseModel):
    ...
    # Location filtering - cities list or single location for backward compatibility
    cities: list[str] | None = None  # Preferred: list of cities to search (e.g., ["Munich", "Berlin"])
    location: str | None = None  # Deprecated: will be removed; use cities instead
    country: Country | None = Country.USA
    
    # Deprecated: distance-based radius search is no longer supported; use cities instead
