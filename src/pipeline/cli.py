...
app = typer.Typer(help="Pipeline runner: jobs_raw → jobs_enriched → job_metrics.")
...
def cmd_run(
    ...
    """Run the full pipeline: jobs_raw → jobs_enriched → job_metrics."""
...
def cmd_stage_raw(
    ...
    """Run Stage 1 only: insert/upsert into jobs_raw."""
    rows = _load_rows(file)
    repo = SupabaseRepository(client=PostgrestClient(config=load_config()))
    result = run_stage_raw(repo=repo, rows=rows)
    _print_stage(result)
...
def cmd_stage_enriched(
    ...
    """Run Stage 2 only: enrich SCRAPED rows into jobs_enriched."""
    repo = SupabaseRepository(client=PostgrestClient(config=load_config()))
    copilot_client = CopilotClient(config=load_copilot_config())
...
@app.command("stage-metrics")
def cmd_stage_metrics(
    scraped_count: int = typer.Option(0, "--scraped", min=0, help="Number of scraped rows to record."),
) -> None:
    """Run Stage 3 only: update job_metrics with incremental counts."""
    repo = SupabaseRepository(client=PostgrestClient(config=load_config()))
